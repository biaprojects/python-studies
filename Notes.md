...
## E para juntar a frase com a variável?
Temos algumas opções:
- Concatenação por soma (+):  
`print("Hello, " + name)`  -> "gruda" uma coisa na outra.
- Concatenação com vírgula (,):  
`print("Hello,", name)`  -> irá incluir, automaticamente, um espaço em branco entre as 2 coisas.
- Print formatado:  
`print(f"Hello, {name}")`  -> inclui a variável entre chaves, indicando a posição que ela ficará.
